...
visualize_temperature <- function(temp_data, city) {
  # Change these colors
  my_colors <- c("blue", "red", "green", "orange", "purple", "pink", "yellow")
  
  # Create separate line plots for each SSP scenario
  plots_temp <- lapply(c("SSP245", "SSP585"), function(ssp) {
    temp_data %>%
      filter(SSP == ssp) %>%
      mutate(Variable = as.factor(Variable)) %>%
      ggplot(aes(x = date, y = Temperature, color = Variable, linetype = Variable)) +
      geom_line(show.legend = TRUE) + 
      labs(title = paste("Temperature in", city, "for", ssp),
           x = "Month",
           y = "Temperature",
           color = "Variable",
           linetype = "Variable") +
      theme_minimal() +
      theme(legend.position = "bottom",  # Remove legend from the plot area
            axis.text.x = element_text(angle = 45, hjust = 1),
            plot.title = element_text(hjust = 0.5)) +
      scale_x_date(date_breaks = "1 month", date_labels = "%b") +
      scale_color_manual(values = my_colors[1:length(unique(temp_data$Variable))]) +
      scale_linetype_manual(values = rep(c("solid", "dashed"), length.out = length(unique(temp_data$Variable))))
  })
  multiplot <- cowplot::plot_grid(plotlist = plots_temp, ncol = 1)
  print(multiplot)
}

visualize_precipitation <- function(precip_data, city) {
  # Define colors
  my_colors <- c("blue", "red", "green", "orange", "purple", "pink", "yellow")
  
  # Create separate line plots for each SSP scenario
  plots_precip <- lapply(c("SSP245", "SSP585"), function(ssp) {
    precip_data %>%
      filter(SSP == ssp) %>%
      mutate(Variable = as.factor(Variable)) %>%
      ggplot(aes(x = date, y = Precipitation, color = Variable, linetype = Variable)) +
      geom_line(show.legend = TRUE) + 
      labs(title = paste("Precipitation in", city, "for", ssp),
           x = "Month",
           y = "Precipitation",
           color = "Variable",
           linetype = "Variable") +
      ...
      theme(legend.position = "bottom",  # Remove legend from the plot area
            ...
            plot.title = element_text(hjust = 0.5)) +
      scale_x_date(date_breaks = "1 month", date_labels = "%b") +
      scale_color_manual(values = my_colors[1:length(unique(precip_data$Variable))]) +
      scale_linetype_manual(values = rep(c("solid", "dashed"), length.out = length(unique(precip_data$Variable))))
  })
  
  # Combine plots into a grid
  multiplot <- cowplot::plot_grid(plotlist = plots_precip, ncol = 1)
...
visualize_cyclones <- function(output, country, city) {
  ...
  # Transform the output into something ggplot-friendly
